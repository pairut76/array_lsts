...
template <typename type>
ArrayList::ArrayList<type>(const ArrayList& a)
...
template <typename type>
type& ArrayList::at(unsigned int index)
{
    return items[index];
}

template <typename type>
const type& ArrayList::at(unsigned int index) const
{
    return items[index];
}
...
template <typename type>
void ArrayList::add(const type& s)
{
    if (sz == cap)
    {
